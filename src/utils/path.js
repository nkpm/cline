...
export function getReadablePath(cwd, relPath) {
	relPath = relPath || ""
...
export async function getCwd(defaultCwd = "") {
...
export async function getWorkspacePath(defaultCwd = "") {
...
export async function isLocatedInWorkspace(pathToCheck = "") {
	const workspacePaths = (await HostProvider.workspace.getWorkspacePaths({})).paths
	for (const workspacePath of workspacePaths) {
...
export function isLocatedInPath(dirPath, pathToCheck) {
...
export async function asRelativePath(filePath) {
	const workspacePaths = await HostProvider.workspace.getWorkspacePaths({})
