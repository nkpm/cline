...
class CheckpointTracker {
	 ...
	 */
	constructor(globalStoragePath, taskId, cwd, cwdHash) {
	 ...
	 */
	static async create(taskId, globalStoragePath, enableCheckpointsSetting) {
	 ...
	 */
	async getDiffSet(lhsHash, rhsHash) {
		const startTime = performance.now()
		const gitPath = await getShadowGitPath(this.globalStoragePath, this.taskId, this.cwdHash)
		const git = simpleGit(path.dirname(gitPath))
	 ...
	 */
	async getDiffCount(lhsHash, rhsHash) {
		const startTime = performance.now()
		const gitPath = await getShadowGitPath(this.globalStoragePath, this.taskId, this.cwdHash)
		const git = simpleGit(path.dirname(gitPath))
...
//# sourceMappingURL=CheckpointTracker.js.map
