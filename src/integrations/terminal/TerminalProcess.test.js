...
function createMockStream(lines = ["test-command", "line1", "line2", "line3"]) {
	return {
...
describe("TerminalProcess (Integration Tests)", () => {
	...
	describe("Controlled output tests", () => {
		...
		it("should correctly filter command echoes based on current implementation", async () => {
			...
			const emitSpy = sandbox.spy(process, "emit")
			await process.run(terminal, "test-command")
			emitSpy.calledWith("line", "test command").should.be.true()
			emitSpy.calledWith("line", "other output").should.be.true()
			emitSpy.calledWith("line", "test-command").should.be.false()
		})
		it("should handle npm run commands", async () => {
			...
			emitSpy.calledWith("line", "> project@1.0.0 build").should.be.true()
			emitSpy.calledWith("line", "> tsc").should.be.true()
			emitSpy.calledWith("line", "files built successfully").should.be.true()
		})
	})
	...
	it("should emit line for remaining buffer when emitRemainingBufferIfListening is called", () => {
		...
		emitSpy.calledWith("line", "test buffer content").should.be.true()
	...
	it("should process buffer and emit lines when newline characters are found", () => {
		...
		emitSpy.calledWith("line", "line 1").should.be.true()
		emitSpy.calledWith("line", "line 2").should.be.true()
		processAny.buffer.should.equal("line 3")
		processAny.emitIfEol(" continued\n")
		emitSpy.calledWith("line", "line 3 continued").should.be.true()
		processAny.buffer.should.equal("")
	})
})
