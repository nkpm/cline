 ...
 */
export async function refreshGroqModels(controller, _request) {
		...
		if (!groqApiKey) {
			...
			if (response.data?.data) {
				...
				for (const rawModel of rawModels) {
					...
					const staticModelInfo = groqModels[rawModel.id]
					const modelInfo = {
 ...
 */
async function readGroqModels(controller) {
	const groqModelsFilePath = path.join(await ensureCacheDirectoryExists(controller), GlobalFileNames.groqModels)
 ...
 */
function isValidChatModel(rawModel) {
 ...
 */
function detectImageSupport(rawModel, staticModelInfo) {
 ...
 */
function generateModelDescription(rawModel, staticModelInfo) {
 ...
 */
async function ensureCacheDirectoryExists(controller) {
	const cacheDir = path.join(controller.context.globalStorageUri.fsPath, "cache")
	await fs.mkdir(cacheDir, { recursive: true })
	return cacheDir
}
//# sourceMappingURL=refreshGroqModels.js.map
