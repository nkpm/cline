 ...
 */
export async function refreshHuggingFaceModels(controller, _request) {
	...
	try {
		...
		if (response.data?.data) {
			...
			for (const rawModel of rawModels) {
				const providersList = rawModel.providers?.map((provider) => provider.provider)?.join(", ")
				...
				if (rawModel.id in huggingFaceModels) {
					const staticModel = huggingFaceModels[rawModel.id]
...
//# sourceMappingURL=refreshHuggingFaceModels.js.map
