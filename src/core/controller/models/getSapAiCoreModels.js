...
import { SapAiCoreModelsResponse } from "@/shared/proto/cline/models"
 ...
 */
async function fetchAiCoreModelsAndOrchestration(accessToken, baseUrl, resourceGroup) {
	if (!accessToken) {
	...
	try {
		...
		const runningDeployments = deployments.filter((deployment) => deployment.targetStatus === "RUNNING")
		// Check for orchestration deployment
		const orchestrationAvailable = runningDeployments.some((deployment) => deployment.scenarioId === "orchestration")
 ...
 */
export async function getSapAiCoreModels(controller, request) {
	try {
...
//# sourceMappingURL=getSapAiCoreModels.js.map
