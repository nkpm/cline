...
export async function checkpointRestore(controller, request) {
	...
	if (request.number) {
		...
		await controller.task?.restoreCheckpoint(request.number, request.restoreType, request.offset)
	}
	return Empty.create({})
}
//# sourceMappingURL=checkpointRestore.js.map
