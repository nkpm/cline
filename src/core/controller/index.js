...
export class Controller {
	...
	constructor(context, id) {
		...
		this.stateManager.onPersistenceError = async ({ error }) => {
    ...
    */
	...
	async togglePlanActMode(modeToSwitchTo, chatContent) {
	...
	async fetchMcpMarketplaceFromApiRPC(silent = false) {
		try {
			const response = await axios.get(`${clineEnvConfig.mcpBaseUrl}/marketplace`, {
				headers: {
					"Content-Type": "application/json",
	...
	async readVercelAiGatewayModels() {
	...
	async getStateToPostToWebview() {
		...
		return {
			...
			isNewUser,
			welcomeViewCompleted: welcomeViewCompleted, // Can be undefined but is set to either true or false by the migration that runs on extension launch in extension.ts
