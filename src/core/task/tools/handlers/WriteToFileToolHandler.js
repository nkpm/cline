...
export class WriteToFileToolHandler {
	validator
	name = "write_to_file" // This handler supports write_to_file, replace_in_file, and new_rule
	...
	async execute(config, block) {
		...
		const relPath = block.params.path
		const content = block.params.content // for write_to_file and new_rule
		let diff = block.params.diff // for replace_in_file
		...
		try {
			...
			if (diff) {
				...
				try {
					newContent = await constructNewFileContent(diff, config.services.diffViewProvider.originalContent || "", true)
				} catch (error) {
...
//# sourceMappingURL=WriteToFileToolHandler.js.map
