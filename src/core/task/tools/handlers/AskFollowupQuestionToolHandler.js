...
export class AskFollowupQuestionToolHandler {
	name = "ask_followup_question"
	supportedTools = ["ask_followup_question"]
	...
	async handlePartialBlock(block, uiHelpers) {
		...
		await uiHelpers.ask("followup", JSON.stringify(sharedMessage), block.partial).catch(() => {})
	}
	async execute(config, block) {
		...
		try {
			...
			if (optionsRaw && text && options.includes(text)) {
				...
				const lastFollowupMessage = findLast(clineMessages, (m) => m.ask === "followup")
...
//# sourceMappingURL=AskFollowupQuestionToolHandler.js.map
