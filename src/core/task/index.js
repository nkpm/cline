...
export class Task {
	...
	constructor(
		...
		const effectiveApiConfiguration = {
			...apiConfiguration,
			ulid: this.ulid,
			onRetryAttempt: async (attempt, maxRetries, delay, error) => {
				const clineMessages = this.messageStateHandler.getClineMessages()
				const lastApiReqStartedIndex = findLastIndex(clineMessages, (m) => m.say === "api_req_started")
				if (lastApiReqStartedIndex !== -1) {
					try {
						const currentApiReqInfo = JSON.parse(clineMessages[lastApiReqStartedIndex].text || "{}")
	...
	updateStrictPlanMode(strictPlanModeEnabled) {
		this.toolExecutor.updateStrictPlanModeEnabled(strictPlanModeEnabled)
	}
	updateUseAutoCondense(useAutoCondense) {
	...
	async resumeTaskFromHistory() {
		...
		const newUserContent = [...modifiedOldUserContent]
		const agoText = (() => {
	...
	async executeCommandInNode(command) {
		try {
			...
			return [
				false,
				`Command executed${wasTerminated ? " (terminated after 30s)" : ""} with exit code ${result.exitCode}.${output.length > 0 ? `\nOutput:\n${output}` : ""}`,
			]
		} catch (error) {
	 ...
	 */
	async migrateDisableBrowserToolSetting() {
		const config = vscode.workspace.getConfiguration("cline")
		const disableBrowserTool = config.get("disableBrowserTool")
	...
	async *attemptApiRequest(previousApiReqIndex) {
		...
		const preferredLanguage = getLanguageKey(this.preferredLanguage)
		...
		try {
			...
			if (isContextWindowExceededError && !this.taskState.didAutomaticallyRetryFailedApiRequest) {
				...
				if (manualRetryApiReqIndex !== -1) {
					const clineMessages = this.messageStateHandler.getClineMessages()
					const currentApiReqInfo = JSON.parse(clineMessages[manualRetryApiReqIndex].text || "{}")
					delete currentApiReqInfo.streamingFailedMessage
	...
	async recursivelyMakeClineRequests(userContent, includeFileDetails = false) {
		...
		if (
			...
			try {
				...
				let checkpointsWarningTimer = null
	...
	async loadContext(userContent, includeFileDetails = false, useCompactPrompt = false) {
