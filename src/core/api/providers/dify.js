export class DifyHandler {
	...
	convertMessagesToQuery(systemPrompt, messages) {
	 ...
	 */
	async uploadFile(file, filename, user = "cline-user") {
	 ...
	 */
	async stopGeneration(taskId, user = "cline-user") {
	 ...
	 */
	async getConversationHistory(conversationId, user = "cline-user", firstId, limit = 20) {
	 ...
	 */
	async getConversations(user = "cline-user", lastId, limit = 20, sortBy = "-updated_at") {
	 ...
	 */
	async deleteConversation(conversationId, user = "cline-user") {
	 ...
	 */
	async renameConversation(conversationId, user = "cline-user", name, autoGenerate = false) {
		const body = { user, auto_generate: autoGenerate }
		if (name) {
			body.name = name
		}
	 ...
	 */
	async submitMessageFeedback(messageId, rating, content, user = "cline-user") {
		const body = { rating, user }
		if (content) {
