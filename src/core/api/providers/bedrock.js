...
export class AwsBedrockHandler {
	...
	static async withTempEnv(updateEnv, fn) {
	 ...
	 */
	async *createDeepseekMessage(systemPrompt, messages, modelId, model) {
	 ...
	 */
	formatDeepseekR1Prompt(systemPrompt, messages) {
	 ...
	 */
	estimateInputTokens(systemPrompt, messages) {
	 ...
	 */
	async *executeConverseStream(command, modelInfo) {
		try {
			const client = await this.getBedrockClient()
			const response = await client.send(command)
			if (response.stream) {
	 ...
	 */
	*handleBedrockStreamError(chunk) {
	 ...
	 */
	prepareSystemMessages(systemPrompt, enableCaching) {
		if (!systemPrompt) {
			return undefined
		}
		if (enableCaching) {
	 ...
	 */
	getInferenceConfig(modelInfo, modelType) {
	 ...
	 */
	async *createAnthropicMessage(systemPrompt, messages, modelId, model, enable1mContextWindow) {
	 ...
	 */
	formatMessagesForConverseAPI(messages) {
	...
	processImageContent(item) {
		...
		try {
			if (typeof item.source.data === "string") {
				...
				imageData = new Uint8Array(Buffer.from(item.source.data))
			} else {
	 ...
	 */
	applyCacheControlToMessages(messages, lastUserMsgIndex, secondLastMsgUserIndex) {
	 ...
	 */
	async *createNovaMessage(systemPrompt, messages, modelId, model) {
	 ...
	 */
	async *createOpenAIMessage(systemPrompt, messages, modelId, model) {
		// Get Bedrock client with proper credentials
		const client = await this.getBedrockClient()
...
__decorate([withRetry({ maxRetries: 4 })], AwsBedrockHandler.prototype, "createMessage", null)
