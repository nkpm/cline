...
export const formatResponse = {
	...
	processFirstUserMessageForTruncation: (originalContent) => {
	...
	toolError: (error) => `The tool execution failed with the following error:\n<error>\n${error}\n</error>`,
	clineIgnoreError: (path) =>
...
(This is an automated message, so do not respond to it conversationally.)`,
	...
	autoApprovalMaxReached: (feedback) =>
	...
	toolResult: (text, images, fileString) => {
	...
	formatFilesList: (absolutePath, files, didHitLimit, clineIgnoreController) => {
		const sorted = files
			.map((file) => {
	...
	taskResumption: (mode, agoText, cwd, wasRecent, responseText, hasPendingFileContextWarnings) => {
...
(Remember: If it seems the user wants you to use tools only available in Act Mode, you should ask the user to "toggle to Act mode" (use those words) - they will have to manually do this themselves with the Plan/Act toggle button below. You do not have the ability to switch to Act Mode yourself, and must wait for the user to do it themselves once they are satisfied with the plan. You also cannot present an option to toggle to Act mode, as this will be something you need to direct the user to do manually themselves.)`
	...
	fileEditWithUserChanges: (relPath, userEdits, autoFormattingEdits, finalContent, newProblemsMessage) =>
		...
		`${newProblemsMessage}`,
	fileEditWithoutUserChanges: (relPath, autoFormattingEdits, finalContent, newProblemsMessage) =>
		...
		`${newProblemsMessage}`,
	diffError: (relPath, originalContent) =>
	...
	clineIgnoreInstructions: (content) =>
		`# .clineignore\n\n(The following is provided by a root-level .clineignore file where the user has specified files and directories that should not be accessed. When using list_files, you'll notice a ${LOCK_TEXT_SYMBOL} next to files that are blocked. Attempting to access the file's contents e.g. through read_file will result in an error.)\n\n${content}\n.clineignore`,
	clineRulesGlobalDirectoryInstructions: (globalClineRulesFilePath, content) =>
	...
	clineRulesLocalDirectoryInstructions: (cwd, content) =>
	...
	clineRulesLocalFileInstructions: (cwd, content) =>
		`# .clinerules\n\nThe following is provided by a root-level .clinerules file where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${content}`,
	windsurfRulesLocalFileInstructions: (cwd, content) =>
	...
	cursorRulesLocalFileInstructions: (cwd, content) =>
	...
	cursorRulesLocalDirectoryInstructions: (cwd, content) =>
	...
	fileContextWarning: (editedFiles) => {
