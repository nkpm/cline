...
const formatSnapshotError = (snapshotName, differences) => {
	return `
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
const compareStrings = (expected, actual) => {
	if (expected === actual) {
		return null
	}
...
const makeMockProviderInfo = (modelId, providerId = "test") => ({
...
const makeMockContext = (modelId, providerId = "test") => ({
	...baseContext,
...
describe("Prompt System Integration Tests", () => {
				...
				for (const modelId of modelIds) {
					for (const { name: contextName, baseContext } of contextVariations) {
						...
						it(`should generate consistent prompt for ${providerId}/${modelId} with ${contextName} context`, async function () {
							...
							try {
								const prompt = await getSystemPrompt(context)
	...
	describe("Legacy Prompt Compatibility", () => {
		...
		let SYSTEM_PROMPT_NEXT_GEN
		let SYSTEM_PROMPT_GENERIC
		before(async () => {
		...
		const extractSectionTitles = (prompt) => {
		...
		const buildOldPrompt = async (promptType, focusChainEnabled) => {
			const mockMcpHub = {
				getServers: () => [
					{
						name: "test-server",
		...
		it("should compare old and new prompts for section title compatibility", async function () {
			...
			const unique = (arr) => Array.from(new Set(arr.map(normalize)))
