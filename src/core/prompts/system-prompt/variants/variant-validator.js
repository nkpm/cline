...
export class VariantValidator {
	...
	validateRequiredFields(variant, errors) {
		if (!variant.id) {
	...
	validateTemplate(variant, errors, warnings) {
	...
	validateComponents(variant, errors, warnings) {
		...
		if (variant.componentOverrides) {
			const invalidOverrides = Object.keys(variant.componentOverrides).filter(
				(key) => !variant.componentOrder.includes(key),
			)
	...
	validateTools(variant, errors, warnings) {
		if (!variant.tools) {
			return
		}
		...
		if (variant.toolOverrides) {
			const invalidOverrides = Object.keys(variant.toolOverrides).filter((key) => !variant.tools?.includes(key))
			if (invalidOverrides.length > 0) {
				warnings.push(`Tool overrides for unused tools: ${invalidOverrides.join(", ")}`)
			}
		}
	}
	validateBestPractices(variant, warnings) {
 ...
 */
export function isValidVariant(variant, options) {
	return validateVariant(variant, options).isValid
}
//# sourceMappingURL=variant-validator.js.map
