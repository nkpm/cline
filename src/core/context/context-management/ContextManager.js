...
export class ContextManager {
	 ...
	 */
	shouldCompactContextWindow(clineMessages, api, previousApiReqIndex) {
	 ...
	 */
	getContextTelemetryData(clineMessages, api, triggerIndex) {
		...
		if (targetIndex >= 0) {
			...
			if (targetRequest && targetRequest.text) {
				try {
					const { tokensIn, tokensOut, cacheWrites, cacheReads } = JSON.parse(targetRequest.text)
	 ...
	 */
	async triggerApplyStandardContextTruncationNoticeChange(timestamp, taskDirectory, apiConversationHistory) {
	 ...
	 */
	applyFirstUserMessageReplacement(timestamp, apiConversationHistory) {
