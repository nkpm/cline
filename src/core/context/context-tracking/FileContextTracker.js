...
export class FileContextTracker {
	...
	recentlyEditedByCline = new Set()
	constructor(controller, taskId) {
	 ...
	 */
	async addFileToFileContextTracker(context, taskId, filePath, source) {
		try {
	 ...
	 */
	markFileAsEditedByCline(filePath) {
	 ...
	 */
	async detectFilesEditedAfterMessage(messageTs, deletedMessages) {
		const editedFiles = []
		try {
	 ...
	 */
	async storePendingFileContextWarning(files) {
		try {
	 ...
	 */
	async retrievePendingFileContextWarning() {
		try {
			const key = `pendingFileContextWarning_${this.taskId}`
			const files = this.controller.stateManager.getWorkspaceStateKey(key)
			return files
		} catch (error) {
	 ...
	 */
	async retrieveAndClearPendingFileContextWarning() {
		try {
	 ...
	 */
	static async cleanupOrphanedWarnings(context) {
		...
		try {
			...
			const orphanedPendingContextTasks = []
