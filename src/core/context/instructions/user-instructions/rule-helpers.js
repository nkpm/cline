 ...
 */
export async function readDirectoryRecursive(directoryPath, allowedFileExtension, excludedPaths = []) {
	try {
 ...
 */
export async function synchronizeRuleToggles(rulesDirectoryPath, currentToggles, allowedFileExtension = "", excludedPaths = []) {
 ...
 */
export function combineRuleToggles(toggles1, toggles2) {
	return { ...toggles1, ...toggles2 }
}
/**
 * Read the content of rules files
 */
export const getRuleFilesTotalContent = async (rulesFilePaths, basePath, toggles) => {
 ...
 */
export async function ensureLocalClineDirExists(clinerulePath, defaultRuleFilename) {
	try {
 ...
 */
export const createRuleFile = async (isGlobal, filename, cwd, type) => {
	try {
		let filePath
		if (isGlobal) {
			if (type === "workflow") {
 ...
 */
export async function deleteRuleFile(controller, rulePath, isGlobal, type) {
	try {
