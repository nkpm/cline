...
export class ClineAccountService {
	...
	async fetchBalanceRPC() {
		try {
			...
			const data = await this.authenticatedRequest(`/api/v1/users/${me.id}/balance`)
			return data
		} catch (error) {
	...
	async fetchUsageTransactionsRPC() {
		try {
			...
			const data = await this.authenticatedRequest(`/api/v1/users/${me.id}/usages`)
			return data.items
		} catch (error) {
	...
	async fetchPaymentTransactionsRPC() {
		try {
			...
			const data = await this.authenticatedRequest(`/api/v1/users/${me.id}/payments`)
	...
	async fetchMe() {
		try {
			const data = await this.authenticatedRequest(`/api/v1/users/me`)
			return data
		} catch (error) {
	...
	async switchAccount(organizationId) {
		...
		try {
			...
			const _response = await this.authenticatedRequest(`/api/v1/users/active-account`, {
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
				},
