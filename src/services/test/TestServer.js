...
let _taskCompletionResolver = null
// Function to create a new task completion promise
function createTaskCompletionTracker() {
 ...
 */
export function createTestServer(controller) {
			...
			try {
				...
				try {
					...
					try {
						...
						const taskData = taskHistory.taskHistory?.find((t) => t.id === taskId)
