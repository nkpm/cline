...
export const getSpawnFunction = () => childProcess.spawn
export async function executeRipgrepForFiles(rgPath, workspacePath, limit = 5000) {
	return new Promise((resolve, reject) => {
		...
		rl.on("close", () => {
			...
			const dirResults = Array.from(dirSet, (dirPath) => ({
				path: dirPath,
				type: "folder",
...
export async function searchWorkspaceFiles(query, workspacePath, limit = 20, selectedType) {
	try {
