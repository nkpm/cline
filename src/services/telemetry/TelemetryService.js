...
export class TelemetryService {
	...
	telemetryCategoryEnabled = new Map([
	 ...
	 */
	async updateTelemetryState(didUserOptIn) {
	 ...
	 */
	captureTaskCreated(ulid, apiProvider) {
		this.capture({
	 ...
	 * @param apiProvider Optional API provider
	 */
	captureTaskRestarted(ulid, apiProvider) {
		this.capture({
	 ...
	 */
	captureConversationTurnEvent(ulid, provider = "unknown", model = "unknown", source, tokenUsage = {}) {
	 ...
	 */
	captureTokenUsage(ulid, tokensIn, tokensOut, model) {
		this.capture({
	 ...
	 */
	captureSummarizeTask(ulid, modelId, currentTokens, maxContextWindow) {
		this.capture({
	 ...
	 */
	captureTaskFeedback(ulid, feedbackType) {
	 ...
	 */
	captureToolUsage(ulid, tool, modelId, autoApproved, success) {
		this.capture({
	 ...
	 */
	captureMcpToolCall(ulid, serverName, toolName, status, errorMessage, argumentKeys) {
		this.capture({
	 ...
	 */
	captureCheckpointUsage(ulid, action, durationMs) {
	 ...
	 */
	captureDiffEditFailure(ulid, modelId, errorType) {
		this.capture({
	 ...
	 */
	captureModelSelected(model, provider, ulid) {
		this.capture({
	 ...
	 */
	captureBrowserToolStart(ulid, browserSettings) {
	 ...
	 */
	captureBrowserToolEnd(ulid, stats) {
		if (!this.isCategoryEnabled("browser")) {
			return
		}
		this.capture({
	 ...
	 */
	captureBrowserError(ulid, errorType, errorMessage, context) {
		if (!this.isCategoryEnabled("browser")) {
			return
		}
		this.capture({
	 ...
	 */
	captureOptionSelected(ulid, qty, mode) {
		this.capture({
	 ...
	 */
	captureOptionsIgnored(ulid, qty, mode) {
		this.capture({
	 ...
	 */
	captureGeminiApiPerformance(ulid, modelId, data) {
		this.capture({
	 ...
	 */
	captureModelFavoritesUsage(model, isFavorited) {
		this.capture({
	...
	captureButtonClick(button, ulid) {
		this.capture({
	 ...
	 */
	captureFocusChainToggle(enabled) {
	 ...
	 */
	captureFocusChainProgressFirst(ulid, totalItems) {
		if (!this.isCategoryEnabled("focus_chain")) {
			return
		}
		this.capture({
	 ...
	 */
	captureFocusChainProgressUpdate(ulid, totalItems, completedItems) {
		if (!this.isCategoryEnabled("focus_chain")) {
			return
		}
		this.capture({
	 ...
	 */
	captureFocusChainIncompleteOnCompletion(ulid, totalItems, completedItems, incompleteItems) {
		if (!this.isCategoryEnabled("focus_chain")) {
			return
		}
		this.capture({
	 ...
	 * @param ulid Unique identifier for the task
	 */
	captureFocusChainListOpened(ulid) {
		if (!this.isCategoryEnabled("focus_chain")) {
			return
		}
		this.capture({
	 ...
	 * @param ulid Unique identifier for the task
	 */
	captureFocusChainListWritten(ulid) {
		if (!this.isCategoryEnabled("focus_chain")) {
			return
		}
		this.capture({
	 ...
	 */
	captureSlashCommandUsed(ulid, commandName, commandType) {
		this.capture({
	 ...
	 */
	captureClineRuleToggled(ulid, ruleFileName, enabled, isGlobal) {
	 ...
	 */
	captureAutoCondenseToggle(ulid, enabled, modelId) {
		this.capture({
	 ...
	 */
	captureTaskInitialization(ulid, taskId, durationMs, hasCheckpoints) {
		this.capture({
