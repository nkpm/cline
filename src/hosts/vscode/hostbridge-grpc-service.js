...
export class ServiceRegistry {
	 ...
	 */
	registerMethod(methodName, handler, metadata) {
	 ...
	 */
	async handleRequest(method, message) {
	 ...
	 */
	async handleStreamingRequest(method, message, responseStream, requestId) {
...
export function createServiceRegistry(serviceName) {
	...
	return {
		registerMethod: (methodName, handler, metadata) => registry.registerMethod(methodName, handler, metadata),
		handleRequest: (method, message) => registry.handleRequest(method, message),
		handleStreamingRequest: (method, message, responseStream, requestId) =>
			registry.handleStreamingRequest(method, message, responseStream, requestId),
		isStreamingMethod: (method) => registry.isStreamingMethod(method),
	}
}
//# sourceMappingURL=hostbridge-grpc-service.js.map
