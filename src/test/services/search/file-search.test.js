...
describe("File Search", () => {
	...
	beforeEach(() => {
		...
		const spawnWrapper = (command, options) => spawnStub(command, options)
		...
		sandbox.stub(fs.promises, "lstat").resolves({ isDirectory: () => false })
	...
	describe("OrderbyMatchScore", () => {
		it("should prioritize results with fewer gaps between matched characters", () => {
			const mockItemA = { item: {}, positions: new Set([0, 1, 2, 5]), start: 0, end: 5, score: 0 }
			const mockItemB = { item: {}, positions: new Set([0, 2, 4, 6]), start: 0, end: 6, score: 0 }
