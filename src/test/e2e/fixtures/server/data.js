...
export class ClineDataMock {
	...
	getMockBalance(userId) {
		return {
			balance: 100000, // Sufficient credits for testing
			userId,
		}
	}
	getMockOrgBalance(organizationId) {
		return {
			balance: 500.0,
			organizationId,
		}
	}
	getMockUsageTransactions(userId, orgId, max = 5) {
	...
	getMockPaymentTransactions(creatorId, max = 5) {
		const transactions = []
		const currentTime = new Date().toISOString()
		for (let i = 0; i < max; i++) {
