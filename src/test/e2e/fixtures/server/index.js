...
export class ClineApiServerMock {
	server
	static globalSharedServer = null
	static globalSockets = new Set()
	currentUser = null
	userBalance = 100.5 // Default sufficient balance
	orgBalance = 500.0
	userHasOrganization = false
	generationCounter = 0
	API_USER = new ClineDataMock("personal")
	constructor(server) {
		this.server = server
	}
	// Test helper methods
		...
		for (const [baseRoute, methods] of Object.entries(E2E_REGISTERED_MOCK_ENDPOINTS)) {
			...
			for (const endpoint of methodEndpoints) {
				...
				if (match) {
					...
					const paramNames = []
					const paramRegex = /\{([^}]+)\}/g
					let paramMatch = paramRegex.exec(fullPattern)
	...
	static async startGlobalServer() {
		...
		const server = createServer((req, res) => {
			...
			const sendApiResponse = (data, status = 200) => {
				console.log(`API Response: ${JSON.stringify(data)}`)
				sendJson({ success: true, data }, status)
			}
			const sendApiError = (error, status = 400) => {
		...
		await new Promise((resolve, reject) => {
			server.listen(E2E_API_SERVER_PORT, (error) => {
				if (error) {
