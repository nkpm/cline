...
export function startProtobusService(controller) {
...
function getProtobusServiceNames(packageDefinition) {
 ...
 */
function wrapHandler(handler, controller) {
...
function wrapStreamingResponseHandler(handler, controller) {
	return async (call) => {
		try {
			const requestId = call.metadata.get("request-id").pop()?.toString()
			log(`ProtoBus gRPC streaming request: ${call.getPath()}`)
			const responseHandler = (response, isLast, _sequenceNumber) => {
				try {
...
function createHealthClient(address) {
	const healthDef = protoLoader.loadSync(health.protoPath)
	const grpcObj = grpc.loadPackageDefinition(healthDef)
...
async function checkHealthOnce(client) {
...
export async function waitForHostBridgeReady(timeoutMs = 60000, intervalMs = 500, address) {
...
//# sourceMappingURL=protobus-service.js.map
