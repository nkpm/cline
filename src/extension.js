...
export async function activate(context) {
	...
	const sidebarWebview = await initialize(context)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("cline.plusButtonClicked", async (webview) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("cline.mcpButtonClicked", (webview) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("cline.settingsButtonClicked", (webview) => {
			const isSidebar = !webview
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("cline.historyButtonClicked", async (webview) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("cline.accountButtonClicked", (webview) => {
    ...
    */
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("cline.addToChat", async (range, diagnostics) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("cline.fixWithCline", async (range, diagnostics) => {
			const context = await getContextForCommand(range, diagnostics)
			if (!context) {
				return
			}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("cline.explainCode", async (range) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("cline.improveCode", async (range) => {
			const context = await getContextForCommand(range)
			if (!context) {
				return
			}
		...
		vscode.commands.registerCommand("cline.focusChatInput", async () => {
			...
			if (activeWebview) {
				...
				if (tabInstances.length > 0) {
					...
					activeWebview = WebviewProvider.getSidebarInstance()
					if (!activeWebview) {
						// Last resort: create new tab
						activeWebview = await openClineInNewTab()
					}
				}
			}
			// Send focus event
...
function setupHostProvider(context) {
	console.log("Setting up vscode host providers...")
	const createWebview = (type) => new VscodeWebviewProvider(context, type)
